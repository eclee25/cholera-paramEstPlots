...
## Name: Elizabeth Lee
## Date: 8/21/16
## Function: Create violin plots for NimBIOS Cholera project parameter estimates -- by parameter, model, type of data -- as a function of percent deviation from the true parameters (cleaned up from v1) -- weekly data for supplement
## Filenames: weekly_param_est_alldata_100d_3y.csv
## Data Source: 
...
print(param.plot2)
ggsave(sprintf("weekly_percTrueDev_byParam%s.%s", code1, ext), param.plot2, width=w, height=h)
...
ggsave(sprintf("weekly_percTrueDev_byModeldata%s.%s", code1, ext), simdata.plot2, width=w, height=h)
...
ggsave(sprintf("weekly_percTrueDev_byNoise%s.%s", code1, ext), noise.plot2, width=w, height=h)
