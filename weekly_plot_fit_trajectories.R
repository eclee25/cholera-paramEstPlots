...
## Name: Elizabeth Lee
## Date: 8/21/16
## Function: plot 100 day fits in ggplot; rows represent fitting model, columns represent noise type, separate plots for informed and naive starting parameters, weekly data
## Filenames: weekly_trajectory_fits_100d.csv, weekly_trajectory_fits_3yr.csv
## Data Source: 
## Notes: 
## 
## useful commands:
...
for (starts in starting.order) {
  ...
  ggsave(sprintf("weekly_allFits_%s_%sStarting.%s", settings$length, starts, ext), fitP.full, width = w, height = h, dpi = dp)
} 
# exported 8/21/16
