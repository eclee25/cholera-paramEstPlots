...
#### import data ################################
imported <- read_csv("all_forecast_data.csv", col_types = "cccciidddd") %>%
...
#### plot preparation ################################
setwd("../../../../paper draft/figures/forecastingplots")
...
pltFunc <- function(dataset){
  dummyP <- ggplot(dataset, aes(x = t, y = forecastData)) +
    ...
    scale_size_identity(name = label.obs, labels = c(""), guide = "legend") +
    geom_line(aes(colour = forecasting_model), linetype = lty, size = sz, alpha = apha) +
    scale_colour_manual(name = "forecasting model", breaks = levels(dataset$forecasting_model), values = model.colors) +
    geom_line(aes(y = TrueClean, alpha = 1), color = 'black', size = sz*2/3, linetype = lty) +
    scale_alpha_identity(name = label.tru, labels = c(""), guide = "legend") +
...
forecP.full <- pltFunc(imported)
forecP.3050 <- pltFunc(imported %>% filter(obsDays != "10 days"))
ggsave(sprintf("allForecasts_informedStarting.pdf"), forecP.full, width = w, height = h, dpi = dp)
ggsave(sprintf("obs3050Fits_informedStarting.pdf"), forecP.3050, width = w, height = h, dpi = dp)
# saved 11/23/15
